...
window.addEventListener('load', function() {
  ...
  if (!sbClient) return;
  sbClient.from('promotions').select('*').eq('active', true).order('created_at', {ascending:false}).limit(1)
    .then(function(r) {
      if (!r.data || !r.data.length) return;
      var p = r.data[0];
      var overlay = document.createElement('div');
      overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.78);z-index:99999;display:flex;align-items:center;justify-content:center;padding:20px;';
      var box = document.createElement('div');
      box.style.cssText = 'background:linear-gradient(135deg,#0a1e3d,#071020);border:2px solid rgba(255,180,0,0.55);border-radius:18px;max-width:540px;width:100%;padding:20px;text-align:center;box-shadow:0 8px 48px rgba(255,180,0,0.22);max-height:90vh;overflow-y:auto;';
      var img = p.image_url ? '<img src="'+p.image_url+'" style="width:100%;max-height:320px;object-fit:contain;border-radius:12px;margin-bottom:14px;display:block;">' : '';
      box.innerHTML = img
        + '<div style="font-size:1.25rem;font-weight:900;color:#ffb300;margin-bottom:10px;">'+p.title+'</div>'
        + '<div style="font-size:0.92rem;color:#e0f0ff;line-height:1.6;white-space:pre-wrap;word-break:break-word;margin-bottom:22px;">'+p.message+'</div>'
        + '<button id="promo-close-btn" style="background:linear-gradient(90deg,#ffb300,#ff8c00);color:#000;border:none;border-radius:10px;padding:10px 32px;font-weight:900;font-size:1rem;cursor:pointer;">OK, j\'ai compris !</button>';
      overlay.appendChild(box);
      document.body.appendChild(overlay);
      document.getElementById('promo-close-btn').addEventListener('click', function(){ overlay.remove(); });
